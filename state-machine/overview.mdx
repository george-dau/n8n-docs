...
Configure this via the `zone_emit_mode` and `heartbeat_seconds` fields on [`POST /api/subscriptions/{id}`](/state-machine/api-reference#post-apisubscriptionsid), or via the **Zone Emit Mode** / **Heartbeat (Seconds)** parameters on the [Detection Webhook Trigger](/nodes/detection-webhook-trigger).
