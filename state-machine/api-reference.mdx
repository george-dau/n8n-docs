...
**Zone emit mode (optional, zone state only):**

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `zone_emit_mode` | String | `"all"` | `"all"` emits `zone_updated` on every detection in an occupied zone. `"composition"` emits only when the subscription's filtered tag → count composition changes, plus a periodic heartbeat. Composition mode requires `signal_type: "zone_state"` and `mode: "streaming"` — other combinations are rejected with `400`. |
| `heartbeat_seconds` | Number | `30` | Maximum seconds between emissions for an unchanged composition. Only meaningful when `zone_emit_mode` is `"composition"`. Must be `>= 0`; `0` falls back to the default of 30. Negative values are rejected with `400`. |

See [Composition emit mode](/state-machine/overview#composition-emit-mode) for the full suppression and heartbeat behavior.
...
| `credentials.ws_url` | String | No | Accepted but unused by this endpoint — this is an HTTP-only query, not a subscription. |
...
| `time_match` | String | No | `started_in` (default) — chronicle's start falls within `[start_time, end_time)`; or `overlap` — chronicle's `[start, end]` interval overlaps `[start_time, end_time)`, with an open chronicle treated as extending to now (producers that never close chronicles will match their full backlog in any window) |
