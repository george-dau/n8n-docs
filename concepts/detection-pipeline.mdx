...
Your n8n workflow receives the enriched state and runs your business logic. A single workflow execution handles one signal — either one track state update or one zone state update. The purpose of the workflow is to **distill high-volume detection data into actionable events** on the Worlds platform.
