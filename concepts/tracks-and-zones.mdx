...
The camera samples at a fixed frame rate (e.g., 10 fps), producing individual **detections** — each with a bounding box, position, and timestamp. The state machine correlates these detections into a single track and enriches the data over time: velocity is calculated after the second detection, zone intersections appear when the object enters a zone, and dwell time accumulates as it stays.
