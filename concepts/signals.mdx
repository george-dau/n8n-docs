...
Fired on **any newer change** to a chronicle the state machine already knows about, including edits made after the chronicle has closed.
...
<Note>
**Dedup guarantee:** the state machine emits exactly one `chronicle_created` per chronicle ID. Duplicate or out-of-order updates — where the chronicle's `updatedAt` is not newer than the last stored state — are suppressed. This lifecycle state survives state machine restarts.
</Note>
...
| `timestamp` | The chronicle's `updatedAt` timestamp (ISO 8601) |
