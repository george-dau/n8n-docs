...
This step uses the built-in **Basic LLM Chain** node with an image input. The model analyzes the still image and returns structured data:
...
2. **As a check gate** (advanced pattern) — feed the `congestion_boolean` into an **IF** node to gate whether the event is created. This adds AI confirmation to reduce false positives.
