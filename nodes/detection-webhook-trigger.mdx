...
These parameters appear only when mode is set to Streaming and signal type is set to Zone State.
...
| **Heartbeat (Seconds)** | Number | 30 | Shown only when **Zone Emit Mode** is *Composition Changes Only*. Maximum seconds between emissions while a zone stays occupied with an unchanged composition (range: 5–3600). Doubles as the retry cadence for workflows that dropped or refuted a previous message. |
