...
Choose which lifecycle signals this trigger delivers. **Trigger On** is a multi-select — pick one or more of `chronicle_created`, `chronicle_updated`, `chronicle_closed`. The default is **Chronicle Created** only; select additional signals to receive more of the lifecycle in the same workflow.

### Chronicle lifecycle signals
...
| `chronicle_updated` | Any newer change to a chronicle the state machine already knows about, including edits made after the chronicle closed |
...
<Note>
**Dedup guarantee**: the state machine emits exactly one `chronicle_created` per chronicle ID. Duplicate or out-of-order updates — where the chronicle's `updatedAt` is not newer than the last state the state machine stored — are suppressed. This lifecycle state survives state machine restarts.
</Note>

## Filtering

Configure which chronicles this trigger receives. Every filter is a list; within a single filter, values are matched with **OR** (any value matches). Across different filters, matches are combined with **AND**. An empty filter matches all chronicles.

| Parameter | Type | Description |
| --- | --- | --- |
| **Chronicle IDs** | Multi-select | Specific chronicle IDs to receive updates for |
| **Chronicle Producer IDs** | Multi-select | Only chronicles created by these producers |
...
| **Labels** | Multi-select | Match against the chronicle's labels. Case-insensitive. |
| **Locations** | Multi-select | Match against the chronicle's location. Case-insensitive. |
| **Statuses** | Multi-select | Match against the chronicle's status. Case-insensitive. |
| **Priorities** | Multi-select | Match against the chronicle's priority. Case-insensitive. |
...
```json
{
  ...
  "trigger_config": {
    ...
    "trigger_on": ["chronicle_created", "chronicle_updated", "chronicle_closed"],
...
| `trigger_config` | Metadata about the trigger that produced this execution: the signal type, the signals selected in **Trigger On**, and the workflow's ID and name |
