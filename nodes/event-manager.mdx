...
<Tabs>
  <Tab title="Create Event">
    ...
    | Parameter | Type | Description |
    | --- | --- | --- |
    | **End Time** | DateTime | If provided, creates the event in a closed state |
...
In the [Track in Zone](/guides/track-in-zone) guide, two Event Manager nodes are used:

1. **Create path:** Creates an event with track metadata, position, camera snapshot, and a processed image upload
2. **Close path:** Closes the event with the end time and calculates duration as metadata: `={{Math.round((new Date($json.track_state.last_seen) - new Date($json.eventOrchestrator.event_start_time)) / 1000)}}`
